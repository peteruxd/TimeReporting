...
export default function ReviewFooter({ status, confidence, hasChanges, onSubmit, onSaveDraft }) {
    return (
            ...
            <div className="max-w-3xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-3 w-full sm:w-auto">
                    <button
                        onClick={onSaveDraft}
                        disabled={!hasChanges}
                        className="flex-1 sm:flex-none px-4 py-2 text-slate-600 font-medium text-sm hover:bg-slate-50 rounded-lg transition-colors border border-transparent hover:border-slate-200 disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                        Save as Draft
                    </button>
                    <button
                        onClick={onSubmit}
                        disabled={status === 'submitted' && !hasChanges}
                    ...
                    >
                        {status === 'submitted' && !hasChanges ? 'Submitted' : (hasChanges && status === 'submitted' ? 'Update Entry' : 'Submit Entry')}
                    </button>
                </div>
